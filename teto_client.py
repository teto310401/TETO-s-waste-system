...
class RemoteClient:
    ...
    def receive_video_stream(self):
        ...
        while self.streaming and self.screen_sock:
            try:
                ...
                if img_len > 0 and img_len < 1024 * 1024 * 10:  # 限制最大10MB
                    # 接收图像数据
                    img_data = b''
                    ...
                    if len(img_data) == img_len:
                        ...
                        if current_time - self.last_fps_time >= 1.0:
                            ...
                            print(f"当前帧率: {fps:.1f} FPS, 已接收 {self.frame_count} 帧")
                            self.frame_count = 0
                            self.last_fps_time = current_time
                    else:
                        print(f"接收数据不完整: {len(img_data)}/{img_len}")
                else:
    ...
    def update_status(self):
        """更新状态栏"""
        if self.streaming:
            self.status_bar.config(text="已连接 - 实时画面传输中")
        elif self.control_sock:
            self.status_bar.config(text="已连接 - 等待视频流...")
        else:
